...
def Privacy_account(args, threshold_epochs, noise_list, iter, dp_mechanism, size):
    # q_s = args['n_selected_client']/args['n_client']
    q_s = 1
    delta_s = 2*args['clip']/size
    if dp_mechanism != 'CRD':
        noise_scale = delta_s*np.sqrt(2*q_s*threshold_epochs*np.log(1/args['delta']))/args['epsilon']
    # elif dp_mechanism == 'CRD':
    #     noise_sum = 0
