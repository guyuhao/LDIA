...
from dataset.utils import auxiliary_test_partition, \
    client_split, generate_loaders, get_dataset_labels, BucketSampler
...
def collate_batch(batch):
    ...
    return pad_sequence(text_list, padding_value=0.0, batch_first=True), torch.tensor(label_list, dtype=torch.float)


def get_dataloader(args, shadow_non_iid=None, quantity=None):
    """
    generate loader of IMDB dataset
...
def get_train_loaders(args, quantity=None):
    ...
    train_iter = IMDB(root=dataset_path, split='train')
    ...
    if vocab is None:
        ...
        if os.path.exists(vocab_path):
            ...
            if not os.path.exists(cache):
                os.mkdir(cache)
            vectors = Vectors(
                name=os.path.join(vector_path, 'glove.6B.100d.txt'),  # torch.Size([2196017, 300])
                cache=cache)
            counter = Counter()
    ...
    args['pad_idx'] = vocab.stoi['<pad>']
    args['unk_idx'] = vocab.stoi['<unk>']
...
def get_test_auxiliary_loaders(args, shadow_non_iid=None):
    ...
    """
    batch_size = args['target_batch_size']
    test_iter = IMDB(root=dataset_path, split='test')
    ...
    auxiliary_loader = generate_loaders(
        ...
        samplers=[BucketSampler(dataset=auxiliary_dataset, batch_size=batch_size, tokenizer=tokenizer)])[0]
    probe_loader = generate_loaders(
        [probe_dataset], 1, shuffle=False, collate_fn=collate_batch,
        samplers=[BucketSampler(dataset=probe_dataset, batch_size=batch_size, tokenizer=tokenizer)])[0]
    test_loader = generate_loaders(
        [test_dataset], 1, shuffle=False, collate_fn=collate_batch,
        samplers=[BucketSampler(dataset=test_dataset, batch_size=batch_size, tokenizer=tokenizer)])[0]
